import { WatermarkOptions, AnyObj } from './types/interface'
import {
  getMutationObserver, getDrawPattern, getStyleStr, genRandomId
} from './utils'
...
class Watermark {
  ...
  render() {
    ...
    if (MutationObserver && this.options.monitor) {
      ...
      this.mutationObserver = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
          ...
          if (
            ...
            && [this.watermarkId, this.watermarkParentId]
              .includes(this.getNodeRandomId(mutation.target))
          ) {
            this.render()
            return
          }
          ...
          if (
            ...
            && [this.watermarkId, this.watermarkParentId]
              .includes((this.getNodeRandomId(mutation.removedNodes[0])))
          ) {
            this.render()
          }
        })
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-expressions
      this.watermarkParentDom.shadowRoot
      && this.mutationObserver.observe(this.watermarkParentDom.shadowRoot, observeOptions)
    }
  }

  getNodeRandomId = (node: Node) => node?.['dataset']?.['1']
...
export { WatermarkOptions, AnyObj }
