...
const encrypt = (str: string): string => window.btoa(decodeURI(encodeURIComponent(str)))

// @ts-ignore
// eslint-disable-next-line no-restricted-properties
export const genRandomId = (prefix = '') => `${encrypt(prefix)}-${(new Date()).getTime()}-${Math.floor(Math.random() * Math.pow(10, 8))}`
